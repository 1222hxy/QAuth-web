...
function getWebAuthnRpId() {
  ...
  if (!hostname || hostname === "unknown") return "oaiusercontent.com";
  return hostname;
}

function getWebAuthnDebugInfo(err) {
  return {
    name: err?.name || "Error",
...
function WebAuthnDemo({ mode }) {
  ...
  async function run() {
    ...
    try {
      ...
      setOutput(JSON.stringify(getWebAuthnDebugInfo(err), null, 2));
    }
  }
        ...
        <Card>
          <div className="p-5 sm:p-8">
            ...
            <div className="mt-5 rounded-2xl bg-zinc-50 p-4 text-sm leading-6 text-zinc-600 ring-1 ring-zinc-200">
              ...
              当前演示会自动读取当前运行环境的 hostname，并把它作为 WebAuthn rpId。这样部署到 Cloudflare Pages、自定义域名或预览域名时，通行密钥域名会自动跟随当前 Origin。
              <br />
              当前运行环境 Origin：<span className="font-mono text-zinc-950">{getRuntimeOrigin()}</span>
            </div>
